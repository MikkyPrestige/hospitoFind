...
const AdminHospitalForm = ({
  ...
  return (
    <form onSubmit={onSubmit} className={styles.adminForm}>
      ...
      <div className={styles.formSection}>
        <label className={styles.sectionLabel}>
          <FiMessageSquare /> Admin Notes & Comments
        </label>
        <textarea
          ...
          value={
            Array.isArray(formData.comments)
              ? formData.comments.join('\n')
              : formData.comments || ''
          }
          onChange={(e) =>
            setFormData({ ...formData, comments: e.target.value.split('\n') })
          }
        />
      </div>
        ...
        <div className={styles.hoursList}>
          ...
          {(!formData.hours || formData.hours.length === 0) && (
            <p className={styles.emptyNote}>
              Click 'Add Day' to specify operating times.
            </p>
          )}
        </div>
      </div>
