...
const SymptomMappings = () => {
    const { mappings, isLoading, fetchMappings, createMapping, updateMapping, deleteMapping } = useSymptomMappings();
                                    ...
                                    <td>
                                        <div className={styles.tagList}>
                                            {m.services.map((svc, i) => (
                                                <span key={i} className={styles.serviceTag}>{svc}</span>
                                            ))}
                                        </div>
                                    </td>
                                    <td className={styles.actions}>
                                        <button onClick={() => openEdit(m)} className={styles.editBtn} title="Edit">
                                            <FiEdit />
                                        </button>
                                        <button onClick={() => handleDelete(m._id, m.symptomKeywords)} className={styles.deleteBtn} title="Delete">
                                            <FiTrash2 />
                                        </button>
                                    </td>
                                </tr>
                ...
                <div className={styles.modalOverlay}>
                    <div className={styles.modalContent}>
                                ...
                                <div className={styles.chipInput}>
                                    {keywords.map((kw, i) => (
                                        <span key={i} className={styles.chip}>
                                            {kw}
                                            <button onClick={() => setKeywords(prev => prev.filter((_, idx) => idx !== i))}>&times;</button>
                                        </span>
                                    ))}
                                    <input
                                        type="text"
                                        ...
                                        onKeyDown={(e) => {
                                            if (e.key === "Enter" && keywordInput.trim()) {
                                                e.preventDefault();
                                                setKeywords(prev => [...prev, keywordInput.trim().toLowerCase()]);
                                                setKeywordInput("");
                                            }
                                        }}
                                    />
                            ...
                            <div className={styles.chipSection}>
                                ...
                                <div className={styles.chipInput}>
                                    {services.map((svc, i) => (
                                        <span key={i} className={styles.chip}>
                                            {svc}
                                            <button onClick={() => setServices(prev => prev.filter((_, idx) => idx !== i))}>&times;</button>
                                        </span>
                                    ))}
                                    <input
                                        type="text"
                                        ...
                                        onKeyDown={(e) => {
                                            if (e.key === "Enter" && serviceInput.trim()) {
                                                e.preventDefault();
                                                setServices(prev => [...prev, serviceInput.trim().toLowerCase()]);
                                                setServiceInput("");
                                            }
                                        }}
                                    />
                        ...
                        <div className={styles.modalFooter}>
                            ...
                            <button onClick={handleSave} disabled={keywords.length === 0 || services.length === 0} className={styles.saveBtn}>
                                Save Mapping
                            </button>
                        </div>
                    </div>
