...
export const SEOHelmet: React.FC<SEOProps> = ({
  ...
  const buildJsonLd = (): Record<string, unknown> | null => {
    switch (schemaType) {
      case 'homepage':
        return {
          ...
          publisher: {
            ...
            logo: {
              '@type': 'ImageObject',
              url: 'https://hospitofind.online/logo.png',
              width: 512,
              height: 512,
            },
          },
          mainEntity: {
            ...
            potentialAction: {
              ...
              target: {
                '@type': 'EntryPoint',
                urlTemplate: `${SITE_URL}/find-hospital?query={search_term_string}`,
              },
      ...
      case 'search':
        return {
          ...
          mainEntity: {
            ...
            numberOfItems: (schemaData as Hospital[] | undefined)?.length || 0,
            itemListElement:
              (schemaData as Hospital[] | undefined)?.map(
                (hospital, index) => ({
                  '@type': 'ListItem',
                  position: index + 1,
                  name: hospital.name,
      ...
      case 'hospital':
        ...
        const h = schemaData as unknown as Hospital
        return {
          '@context': 'https://schema.org',
          '@type': 'Hospital',
          name: h.name,
          ...
          areaServed: h.address?.city
            ? { '@type': 'City', name: h.address.city }
            : undefined,
        }
      ...
      case 'outbreaks':
        return {
          ...
          mainEntity: {
            ...
            name: title,
            numberOfItems:
              (schemaData as Record<string, unknown>[] | undefined)?.length ||
              0,
            itemListElement:
              (schemaData as Record<string, unknown>[] | undefined)?.map(
                (item, index) => ({
                  '@type': 'ListItem',
                  position: index + 1,
      ...
      case 'terms':
        return {
          ...
          about: {
            '@type': 'Thing',
            name:
              schemaType === 'policy' ? 'Privacy Policy' : 'Terms of Service',
            description:
              schemaType === 'policy'
  ...
  const breadcrumbsLd = buildBreadcrumbs(country)
