  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (entryMode === 'markdown') {
      ...
      payload = {
        ...
        services: getValue(/# Services:\s*([\s\S]*?)(?=# Comments)/)
          .split(',')
          .map((s) => s.trim())
          .filter(Boolean),
        comments: [getValue(/# Comments:\s*([\s\S]*?)(?=# Hours|---)/)].filter(
          Boolean
        ),
        hours: [],
        verified: false,
      ...
      payload = {
        ...
        hours: formData.hours
          ? [{ day: 'Standard Operating Hours', open: formData.hours }]
          : [],
        verified: false,
      }
    }
    ...
    if (!payload.name || !payload.address.city || !payload.address.country) {
      setError(
        'Data Integrity Error: Facility Name, City, and Country are strictly required.'
      )
      return
    }
    ...
    <section className={style.editorContainer}>
      <header className={style.editorHeader}>
        ...
        <p className={style.subtitle}>
          Help us expand our trusted network. Ensure data accuracy to assist
          users safely.
        </p>
              ...
              <div className={style.formSection}>
                ...
                <div className={style.inputGroup}>
                  ...
                  <textarea
                    ...
                    onChange={handleFormChange}
                    className={style.inputField}
                    placeholder="e.g., Emergency, Dental, Surgery, Pediatrics"
                  ></textarea>
                </div>
                <div className={`${style.formGrid} ${style.twoCol}`}>
                  <div className={style.inputGroup}>
                    ...
                    <textarea
                      ...
                      onChange={handleFormChange}
                      className={style.inputField}
                      placeholder="Any critical information users should know..."
                    ></textarea>
                  </div>
                  <div className={style.inputGroup}>
                    ...
                    <textarea
                      ...
                      onChange={handleFormChange}
                      className={style.inputField}
                      placeholder="e.g., Mon - Fri: 8:00 AM - 5:00 PM, Emergency 24/7"
                    ></textarea>
                  </div>
                </div>
              </div>
        ...
        {showSuccess && (
          <div className={style.modalOverlay}>
            ...
            >
              ...
              <p>
                Thank you, <strong>{state?.username || 'Contributor'}</strong>.
                The facility data has been securely transmitted for
                verification.
              </p>
              <button onClick={resetState} className={style.closeModalBtn}>
                Submit Another
              </button>
            </motion.div>
          </div>
