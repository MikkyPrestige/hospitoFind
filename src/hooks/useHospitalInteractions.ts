...
export function useHospitalInteractions(
  ...
  const toggleFav = useCallback(
    async (h: Hospital) => {
      ...
      const exists = raw.some((r: Hospital) => r._id === h._id)
      const next = exists
        ? raw.filter((r: Hospital) => r._id !== h._id)
        : [{ ...h }, ...raw].slice(0, 50)
  ...
  const handleExplore = useCallback(
    async (hospital: Hospital) => {
      ...
      const filtered = raw.filter(
        (h: Hospital) => String(h._id) !== String(hospital._id)
      )

      const next = [{ ...hospital, viewedAt: Date.now() }, ...filtered].slice(
        0,
        10
      )
