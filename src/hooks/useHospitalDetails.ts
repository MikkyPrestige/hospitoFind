...
import { UseHospitalDetailsProps, Hospital } from '@/types/hospital'
...
export const useHospitalDetails = ({
    ...
    async (hospitalData: Hospital) => {
      ...
      try {
        ...
        const next = [
          ...
          ...raw.filter((h: Hospital) => h._id !== hospitalData._id),
        ].slice(0, 10)
      ...
      if (accessToken && hospitalData._id) {
        try {
          await axiosPrivate.post('/user/view', {
  ...
  useEffect(() => {
    const fetchHospital = async () => {
      try {
        ...
        const isFav = saved.some(
          (h: Hospital) => h._id === res._id || h.name === res.name
        )
        setIsFavorite(isFav)
        ...
        const message =
          err instanceof Error
            ? err.message
            : 'Failed to retrieve hospital details.'
  ...
  const toggleFavorite = async () => {
    ...
    if (previousState === true) {
      next = saved.filter(
        (h: Hospital) => h._id !== hospital._id && h.name !== hospital.name
      )
    } else {
