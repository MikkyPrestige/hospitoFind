...
export const useOsmImport = (onSuccess?: () => void) => {
  ...
  const importHospitals = async (
    ...
    timersRef.current = [
      ...
      setTimeout(
        () => setLoadingText('Mapping fields and deduplicating...'),
        5500
      ),
    ]
    ...
    try {
      ...
      if (dryRun) {
        ...
        toast.success(
          `Preview complete. ${data.imported} hospitals would be imported.`
        )
      } else {
        setResult(data)
        toast.success(
          `Imported ${data.imported} hospitals. Skipped ${data.skipped} duplicates.`
        )
