      ...
      <div className={style.section}>
        <Motion variants={sectionReveal} className={style.pageHeader}>
          ...
          <p className={style.pageSubtitle}>
            Stay informed about current health concerns, safety protocols, and
            live disease updates worldwide.
          </p>
        </Motion>
        ...
        <div className={style.layout}>
          <main className={style.mainFeed}>
            {loading ? (
              <AnimatedLoader
                message="Scanning global health reports..."
                variant="list"
                count={6}
              />
            ) : error ? (
          ...
          <aside className={style.sidebar}>
            <div className={style.regionCard}>
              <h3>
                <FiGlobe className={style.cardIcon} /> Regional Safety Center
              </h3>
              <p className={style.sidebarHint}>
                Showing protocols for your region
              </p>
              <select
            ...
            <div className={style.safetyCard}>
              <h3>
                <FiShield className={style.cardIcon} /> {activeData.name}{' '}
                Protocols
              </h3>
              <ul className={style.protocolList}>
                {activeData.protocols.map((tip, i) => (
                  <li key={i}>
                    <FiInfo className={style.listIcon} /> {tip}
                  </li>
                ))}
              </ul>
            </div>
            ...
            <div className={style.emergencyCard}>
              <h3>
                <FiPhone className={style.cardIcon} /> {activeData.agency}
              </h3>
              ...
              <p className={style.emergencyNote}>
                Contact local authorities immediately if you are experiencing a
                medical emergency.
              </p>
            </div>
          </aside>
