...
const OsmImport = ({ onSuccess }: { onSuccess?: () => void }) => {
    ...
    const { importHospitals, loading, loadingText, result, dryRunResult, resetImport } = useOsmImport(onSuccess);
                ...
                <div className={styles.resultBox}>
                    <div className={styles.statsGrid}>
                        <div className={`${styles.stat} ${styles.success}`}>
                            <span className={styles.count}>{result?.imported ?? dryRunResult?.imported ?? 0}</span>
                            <span className={styles.label}>{dryRunResult ? "Would Import" : "Imported"}</span>
                            <FiCheckCircle className={styles.statIcon} />
                        </div>
                        <div className={`${styles.stat} ${styles.skipped}`}>
                            <span className={styles.count}>{result?.skipped ?? dryRunResult?.skipped ?? 0}</span>
                    ...
                    {dryRunResult?.preview && (
                        <details className={styles.previewDetails}>
                            <summary>Preview hospitals ({dryRunResult.preview.length})</summary>
                            <ul className={styles.previewList}>
                                {dryRunResult.preview.map((h: any, i: number) => (
                                    <li key={i}>
                                        <strong>{h.name}</strong> — {h.address?.city}, {h.address?.state}
                                    </li>
                                ))}
                            </ul>
                        </details>
