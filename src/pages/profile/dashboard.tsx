...
const Dashboard = () => {
  ...
  const { fetchActivity, removeFavoriteItem, removeHistoryItem } =
    useUserActivity()
  ...
  const [recentlyViewed, setRecentlyViewed] = useState<Hospital[]>([])
  ...
  useEffect(() => {
    ...
    if (routerState?.tab) {
      setTimeout(() => setSelected(routerState.tab!), 0)
      localStorage.setItem('selectedLink', routerState.tab)
    }

    if (routerState?.hospitalContext) {
      setTimeout(() => setSavedHospitalContext(routerState.hospitalContext!), 0)
    }
  ...
  useEffect(() => {
    const hydrateData = async () => {
      ...
      const localRecents = JSON.parse(
        localStorage.getItem(RECENTLY_KEY) || '[]'
      )
      ...
      if (dbData) {
        ...
        localStorage.setItem(
          RECENTLY_KEY,
          JSON.stringify(dbData.recentlyViewed)
        )
        localStorage.setItem(
          WEEKLY_KEY,
          JSON.stringify({
  ...
  const menuItems = useMemo(
    () => [
      ...
      { id: 'settings', label: 'Settings', icon: <FiSettings /> },
      {
        id: 'logout',
        label: 'Log Out',
        icon: <IoIosLogOut className={style.logoutItem} />,
      },
    ],
    []
  )
  ...
  useEffect(() => {
    ...
    if (storedSelectedLink) {
      setTimeout(() => setSelected(storedSelectedLink), 0)
    }
  }, [])
  ...
  const removeFavorite = async (e: React.MouseEvent, id: string) => {
    ...
    if (success) {
      const next = favorites.filter((item: Hospital) => item._id !== id)
      setFavorites(next)
  ...
  const removeRecent = async (e: React.MouseEvent, id: string) => {
    ...
    if (success) {
      const next = recentlyViewed.filter((item: Hospital) => item._id !== id)
  ...
  return (
    <main className={style.dashboard}>
      ...
      <section className={style.mainLayout}>
        <nav
          className={`${style.nav} ${!sidebarExpanded ? style.miniSidebar : ''}`}
        >
              ...
              <header className={style.greetingHeader}>
                <div className={style.greetingText}>
                  ...
                  <p className={style.activityStats}>
                    You have viewed <strong>{recentlyViewed.length}</strong>{' '}
                    facilities recently (<strong>{weeklyViews}</strong> visits
                    this week).
                  </p>
                </div>
              ...
              {!hasResults && (
                <div className={style.overviewGrid}>
                  <div className={style.accordionSection}>
                    ...
                    >
                      <h2>
                        <FiHeart className={style.headerIcon} /> Saved
                        Collections
                      </h2>
                            ...
                            <div className={style.listWrapper}>
                              {favorites.map((h, i) => (
                                <div key={i} className={style.dashboardCard}>
                                  <Link
                                    to={`/hospital/${encodeURIComponent(h.address.country || '')}/${encodeURIComponent(h.address.city || '')}/${h.slug}`}
                                  ...
                                  <button
                                    ...
                                    onClick={(e) => removeFavorite(e, h._id)}
                  ...
                  <div className={style.accordionSection}>
                    ...
                    >
                      <h2>
                        <FiClock className={style.headerIcon} /> Recent History
                      </h2>
                      <div className={style.chevron}>
                          ...
                          {recentlyViewed.length > 0 ? (
                            <div className={style.listWrapper}>
                              {recentlyViewed.map((r, i) => (
                                <div key={i} className={style.dashboardCard}>
                                  <Link
                                    to={`/hospital/${encodeURIComponent(r.address.country || '')}/${encodeURIComponent(r.address.city || '')}/${r.slug}`}
                                    className={style.cardLink}
                                  >
                                    <Avatar
            ...
            <Motion variants={fadeUp} className={style.viewPanel}>
              <div className={style.settingsHub}>
                ...
                <nav className={style.tabsNav}>
                  ...
                  <button
                    className={
                      settingsTab === 'security' ? style.activeTab : ''
                    }
                ...
                <div className={style.tabContent}>
                  <AnimatePresence mode="wait">
                          ...
                          {state.auth0Id ? (
                            <div className={style.socialLoginNotice}>
                              ...
                              <p className={style.noticeBody}>
                                ...
                                <span className={style.providerTag}>
                                  {getProviderName()}
                                </span>
                                .
                              </p>
                              <p className={style.noticeFooter}>
                                To update your password or email, please visit
                                your {getProviderName()} account settings.
                              </p>
                            </div>
                          ) : (
                            <div className={style.cardBody}>
                    ...
                    {settingsTab === 'danger' && (
                      ...
                      >
                        <div
                          className={`${style.settingsCard} ${style.dangerZone}`}
                        >
                          <div className={style.cardHeader}>
                          ...
                          <div className={style.cardBody}>
                            <p className={style.warningMsg}>
                              Warning: This action is permanent. All your
                              submitted data and saved lists will be erased.
                            </p>
                            <DeleteBtn />
                          </div>
                        </div>
