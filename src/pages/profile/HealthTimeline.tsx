...
const FeedbackPrompt = ({ session, onSubmit }: FeedbackPromptProps) => {
  ...
  if (
    ...
    session.rating ||
    submitted ||
    dismissed.includes(session._id)
  )
    return null
  ...
  const handleSubmit = async () => {
    ...
    if (selectedHospital && selectedHospital !== 'none')
      payload.hospitalVisited = selectedHospital
  ...
  return (
    <div className={style.feedbackPrompt}>
      <button
        type="button"
        className={style.feedbackTrigger}
      ...
      {expanded && (
        <div className={style.feedbackForm}>
          <div className={style.formSection}>
            ...
            <div className={style.hospitalPills}>
              {session.matchedHospitals.map((m) => {
                const id =
                  typeof m.hospitalId === 'object' && m.hospitalId !== null
                    ? m.hospitalId._id
                    : String(m.hospitalId)
          ...
          <button
            type="button"
            className={style.feedbackSubmit}
...
const PatternAlert = ({ symptoms }: { symptoms: string[] }) => (
  <div className={style.patternAlert}>
    ...
    <p>
      Repeated pattern: <strong>{symptoms.join(', ')}</strong>. Consider
      discussing this recurrence with a doctor.
    </p>
  </div>
)
        ...
        <div className={style.headerMain}>
          ...
          <div className={style.symptoms}>
            {session.symptoms.map((s) => (
              <span key={s} className={style.symptomTag}>
                {s}
              </span>
            ))}
          </div>
            ...
            <div className={style.ratingDisplay}>
              {[1, 2, 3, 4, 5].map((star) => (
                <FiStar
                  ...
                  size={11}
                  className={
                    session.rating && session.rating >= star
                      ? style.starFilledDisplay
                      : style.starEmptyDisplay
                  }
                />
              ))}
            </div>
        ...
        {session.hospitalVisited && (
          <div className={style.visitedContainer}>
            ...
            <div className={style.visitedBadge}>
              ...
              <span>Visited: </span>
              <Link
                to={`/hospital/${session.hospitalVisited.address?.state}/${session.hospitalVisited.address?.city}/${session.hospitalVisited.slug}`}
                className={style.visitedLink}
              >
...
const HealthTimeline = ({ hospitalContext }: HealthTimelineProps) => {
  ...
  useEffect(() => {
    if (hospitalContext === undefined) return
    const id = setTimeout(() => setActiveContext(hospitalContext ?? null), 0)
  ...
  useEffect(() => {
    ...
    const needsFeedback = history.filter(
      (s) =>
        !s.hospitalVisited &&
        !s.rating &&
        !alreadyPrompted.includes(s._id) &&
        !alreadyDismissed.includes(s._id)
    )

    if (needsFeedback.length > 0) {
      toastShownRef.current = true
      toast.info(
        `Health check: Tell us about your ${needsFeedback.length} recent visit${needsFeedback.length > 1 ? 's' : ''}.`,
        {
          toastId: 'feedback-reminder',
          autoClose: 6000,
          position: 'top-right',
        }
      )
  ...
  if (error) {
    return (
      <div className={style.errorState}>
        <div className={style.errorCircle}>
  ...
  return (
    <div className={style.wrapper}>
      ...
      {repeatedSymptoms.length > 0 && (
        <PatternAlert symptoms={repeatedSymptoms} />
      )}
        ...
        <section className={style.historyCol}>
          ...
          {history.length === 0 ? (
            <div className={style.emptyHistory}>
              ...
              <p>
                Complete a session with our AI assistant to start your health
                timeline.
              </p>
            </div>
          ) : (
