...
const PatternAlert = ({ symptoms }: { symptoms: string[] }) => (
    <div className={style.patternAlert}>
        ...
        <FiAlertCircle size={18} className={style.alertIcon} />
                    ...
                    <div className={style.agentWrapper}>
                        {activeContext?.name && (
                            <div className={style.contextBanner}>
                                <div className={style.contextIcon}>🏥</div>
...
// import { Link } from 'react-router-dom';
// import {
//     FiTrash2, FiMapPin, FiStar, FiAlertCircle,
//     FiActivity, FiChevronDown, FiChevronUp, FiCheck
// } from 'react-icons/fi';
...
// const StarRating = ({ value, onChange }: { value: number | null; onChange: (rating: number) => void; }) => (
//     <div className={style.stars}>
//         {[1, 2, 3, 4, 5].map((star) => (
//             <button
//                 key={star}
...
//             <button type="button" className={style.feedbackTrigger} onClick={() => setExpanded(!expanded)}>
//                 <span className={style.feedbackTriggerDot} />
//                 <span>Did you visit one of these hospitals?</span>
//                 {expanded ? <FiChevronUp size={12} /> : <FiChevronDown size={12} />}
//             </button>

//             {expanded && (
//                 <div className={style.feedbackForm}>
//                     <p className={style.feedbackLabel}>Which hospital did you visit?</p>
//                     <div className={style.hospitalPills}>
//                         {session.matchedHospitals.map((m) => {
//                             const id = typeof m.hospitalId === 'object' && m.hospitalId !== null
//                                 ? m.hospitalId._id : String(m.hospitalId);
...
//                             className={`${style.pill} ${selectedHospital === 'none' ? style.pillSelected : ''}`}
//                             onClick={() => setSelectedHospital('none')}>
//                             {selectedHospital === 'none' && <FiCheck size={11} />}
//                             None of the above
//                         </button>
//                     </div>

//                     <p className={style.feedbackLabel}>How was your experience?</p>
//                     <StarRating value={rating} onChange={setRating} />
...
// const PatternAlert = ({ symptoms }: { symptoms: string[] }) => (
//     <div className={style.patternAlert}>
//         <FiAlertCircle size={15} className={style.alertIcon} />
//         <p>You've reported <strong>{symptoms.join(', ')}</strong> multiple times. Consider booking a specialist appointment.</p>
//     </div>
// );
...
//                     <button type="button" className={style.expandBtn} onClick={() => setExpanded(!expanded)}>
//                         {expanded ? <FiChevronUp size={12} /> : <FiChevronDown size={12} />}
//                         {expanded ? 'Hide' : 'Show'} matched hospitals ({session.matchedHospitals.length})
//                     </button>
//                     {expanded && (
//                         <div className={style.matchedList}>
//                             {session.matchedHospitals.map((m, i) => (
//                                 <div key={i} className={style.matchedItem}>
//                                     <span className={style.matchRank}>#{i + 1}</span>
//                                     <span className={style.matchName}>{m.name}</span>
//                                     <span className={style.matchScore}>{m.matchScore}% match</span>
//                                 </div>
//                             ))}
//                         </div>
//                     )}
...
//             toastShownRef.current = true;
//             toast.info(
//                 `💬 You have ${needsFeedback.length} session${needsFeedback.length > 1 ? 's' : ''} without feedback. Share your experience below!`,
//                 { toastId: 'feedback-reminder', autoClose: 6000, position: 'top-right' }
//             );
//             needsFeedback.forEach((s) => addToLocalList(LS_PROMPTED, s._id));
//         }
...
//     const handleDelete = async (sessionId: string) => {
//         const result = await deleteSession(sessionId);
//         if (result.success) toast.success('Session removed');
//         else toast.error(result.error || 'Failed to delete');
//     };

//     const handleClearAll = async () => {
//         if (!window.confirm('Clear your entire health history? This cannot be undone.')) return;
//         const result = await clearHistory();
//         if (result.success) {
//             toast.success('Health history cleared');
//             localStorage.removeItem(LS_PROMPTED);
//             localStorage.removeItem(LS_DISMISSED);
//         } else {
//             toast.error(result.error || 'Failed to clear history');
...
//                     <FiActivity size={18} className={style.headerIcon} />
//                     <div>
//                         <h2 className={style.title}>Health Timeline</h2>
//                         <p className={style.subtitle}>
//                             {history.length > 0
//                                 ? `${history.length} session${history.length !== 1 ? 's' : ''} recorded`
//                                 : 'Start a conversation below to get matched with a hospital'}
//                         </p>
//                     </div>
//                 </div>
//                 {history.length > 0 && (
...
//                     <span>🏥</span>
//                     <p>
//                         Finding hospitals near <strong>{activeContext.name}</strong>
//                         {activeContext.city && ` in ${activeContext.city}`}
//                         {activeContext.country && `, ${activeContext.country}`}
//                     </p>
//                 </div>
//             )}

//             <div className={style.columns}>
//                 <div className={style.agentCol}>
//                     <p className={style.colLabel}>AI Assistant</p>
//                     <AgentWidget
//                         variant="dashboard"
//                         onSessionComplete={handleSessionComplete}
//                         onStartOver={() => setActiveContext(null)}
...
//                 <div className={style.historyCol}>
//                     {history.length > 0 && <p className={style.colLabel}>Past Sessions</p>}

//                     {history.length === 0 ? (
//                         <div className={style.emptyHistory}>
//                             <HiSparkles size={22} className={style.emptyIcon} />
//                             <p>Your sessions will appear here after you complete a match.</p>
//                         </div>
//                     ) : (
//                         <div className={style.timeline}>
//                             {history.map((session) => (
