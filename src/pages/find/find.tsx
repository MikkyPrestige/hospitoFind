...
const FindHospital = () => {
  ...
  const updateMapMarkers = useCallback(
    (data: Hospital[], mapInstance: mapboxgl.Map) => {
  ...
  }, [map, hospitals, updateMapMarkers])
  ...
  useEffect(() => {
    ...
    const executeUrlSearch = async () => {
      let results: { data: Hospital[]; displayString: string } = {
        data: [],
        displayString: '',
      }
      if (urlCity && urlState) {
    ...
    executeUrlSearch()
  }, [searchParams, map, performSearch, updateMapMarkers])
  ...
  useEffect(() => {
    const id = setTimeout(
      () => {
        ...
        autocompleteHospitals(term)
          .then((data) => setSuggestions(data || []))
          .catch(() => setSuggestions([]))
      },
      term.trim().length < 2 ? 0 : 250
    )
              ...
              {hospitals.length > 0 ? (
                ...
                <Motion variants={fadeUp} className={style.emptyState}>
                  ...
                  <p className={style.emptyStateText}>
                    We couldn’t locate any verified facilities in{' '}
                    <strong>{emptyResultQuery}</strong>.
                    <br />
                    <br />
                    Know a hospital here? Help expand access by adding it via
                    your{' '}
                    <span
